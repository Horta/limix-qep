...
from limix_qep.moments import PoissonMoments


def _test_moments(pm, N, mu, var, lmom0, mu_res, var_res):
    mu = asarray([mu])
    var = asarray([var])
    lmom0n, mu_resn, var_resn = empty_like(mu), empty_like(mu), empty_like(mu)
    NN = empty_like(mu)
    NN[:] = N

    pm.compute(NN, mu / var, 1. / var, lmom0n, mu_resn, var_resn)
...
def test_moments():
    pm = PoissonMoments(100)

    N = 5
    mu = 0.1
    var = 1.2
    lmom0 = -3.409879756112254
    mu_res = 1.284967557325151
    var_res = 0.220834592153251

    _test_moments(pm, N, mu, var, lmom0, mu_res, var_res)

    mu = +10.
    # lmom0 = -36.4957661804
    # mu_res = 1.91509505632
    # var_res = 0.266046261313
    # _test_moments(pm, N, K, mu, var, lmom0, mu_res, var_res)
    #
    # mu = -10.
    # lmom0 = -31.8411020448
