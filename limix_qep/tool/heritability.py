...
def estimate(y, G=None, K=None, QS=None, covariate=None,
    ...
    if isinstance(outcome_type, Bernoulli):
        ...
        klass = BinomialEP
    elif isinstance(outcome_type, Poisson):
        klass = PoissonEP
    else:
        raise Exception("NOT KNOWN LIKELIHOOD")

    logger.debug('Constructing EP.')

    # (self, y, M, Q0, Q1, S0, Q0S0Q0t=None)
    if isinstance(outcome_type, Bernoulli):
        ep = klass(y, covariate, Q[0], Q[1], S[0])
    elif isinstance(outcome_type, Binomial):
        ep = klass(y, outcome_type.ntrials, covariate, Q[0], Q[1], S[0])
    elif isinstance(outcome_type, Poisson):
        ep = klass(y, covariate, Q[0], Q[1], S[0])
    else:
        ep = klass(y, covariate, Q, S, outcome_type=outcome_type)
