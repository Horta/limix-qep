...
class Bernoulli(object):
    ...
    def assert_outcome(self, y):
        y = asarray(y)
        if not all_(logical_or(y == 0.0, y == 1.0)):
...
class Binomial(object):
    ...
    def __init__(self, ntrials, nsamples=None):
        ...
        if isscalar(ntrials):
            ...
            ntrials = full(nsamples, ntrials, dtype=float)
        self._ntrials = asarray(ntrials, float)
    ...
    def __eq__(self, other):
        ...
        if self._ntrials.ndim != other.ntrials.ndim:
            return False
        return all_(self._ntrials == other.ntrials)
...
class Poisson(object):
    ...
    def __eq__(self, other):
        return isinstance(other, Poisson)
