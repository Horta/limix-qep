...
double log_ndtr(double a)
{
    ...
    if (a > -20) {
	return torch_cephes_log(torch_cephes_ndtr(a));
    }
    log_LHS = -0.5 * a * a - torch_cephes_log(-a) - 0.5 * torch_cephes_log(2 * M_PI);

    while (torch_cephes_fabs(last_total - right_hand_side) > DBL_EPSILON) {
	i += 1;
