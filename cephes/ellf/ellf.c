...
if( kind == 3 )
	...
	if( dbd > 0.0 )
		...
		m1 *= m1;
		m1p = 1.0 - m1;
		Kk1 = torch_cephes_ellpk( m1p );
		Kpk1 = torch_cephes_ellpk( m1 );
		q = exp( -PI * Kpk1 / (rn * Kk1) );
		k = torch_cephes_cay(q);
		if( type >= 3 )
			wr = k;
		else
			wr = 1.0/k;
...
k = wc/wr;
m = k * k;
Kk = torch_cephes_ellpk( 1.0 - m );
Kpk = torch_cephes_ellpk( m );
q = exp( -PI * rn * Kpk / Kk );	/* the nome of k1 */
m1 = torch_cephes_cay(q); /* see below */
...
b = 1.0/eps;
phi = atan( b );
u = torch_cephes_ellik( phi, m1p );
printf( "phi %.7e m %.7e u %.7e\n", phi, m1p, u );
/* consistency check on inverse sn */
torch_cephes_ellpj( u, m1p, &sn, &cn, &dn, &phi );
a = sn/cn;
...
if( kind == 3 )
	{
	nz = n/2;
	torch_cephes_ellpj( u, 1.0-m, &sn1, &cn1, &dn1, &phi1 );
	for( i=0; i<ARRSIZ; i++ )
		zs[i] = 0.0;
	for( i=0; i<nz; i++ )
		{	/* zeros */
		a = n - 1 - i - i;
		b = (Kk * a) / rn;
		torch_cephes_ellpj( b, m, &sn, &cn, &dn, &phi );
		lr = 2*np + 2*i;
		zs[ lr ] = 0.0;
		a = wc/(k*sn);	/* k = sqrt(m) */
		zs[ lr + 1 ] = a;
 ...
 */
		...
		cden.i = -C * r.i;
		jt += 1;
		torch_cephes_cdiv( &cden, &cnum, &z[jt] );
		if( r.i != 0.0 )
			{
 ...
 */
		if( kind == 2 )
			...
			cwc.r = c;
		cwc.i = 0.0;
		torch_cephes_cmul( &r, &cwc, &cnum );     /* r wc */
		torch_cephes_csub( &cnum, &torch_cephes_cone, &ca );   /* a = 1 - r wc */
		torch_cephes_cmul( &cnum, &cnum, &b4ac ); /* 1 - (r wc)^2 */
		torch_cephes_csub( &b4ac, &torch_cephes_cone, &b4ac );
		b4ac.r *= 4.0;               /* 4ac */
		b4ac.i *= 4.0;
		cb.r = -2.0 * cgam;          /* b */
		cb.i = 0.0;
		torch_cephes_cmul( &cb, &cb, &cnum );     /* b^2 */
		torch_cephes_csub( &b4ac, &cnum, &b4ac ); /* b^2 - 4 ac */
		torch_cephes_csqrt( &b4ac, &b4ac );
		cb.r = -cb.r;  /* -b */
		cb.i = -cb.i;
		ca.r *= 2.0; /* 2a */
		ca.i *= 2.0;
		torch_cephes_cadd( &b4ac, &cb, &cnum );   /* -b + sqrt( b^2 - 4ac) */
		torch_cephes_cdiv( &ca, &cnum, &cnum );   /* ... /2a */
		jt += 1;
		torch_cephes_cmov( &cnum, &z[jt] );
		if( cnum.i != 0.0 )
			{
			jt += 1;
			z[jt].r = cnum.r;
		...
		if( (r.i != 0.0) || (cnum.i == 0) )
			{
			torch_cephes_csub( &b4ac, &cb, &cnum );  /* -b - sqrt( b^2 - 4ac) */
			torch_cephes_cdiv( &ca, &cnum, &cnum );  /* ... /2a */
			jt += 1;
			torch_cephes_cmov( &cnum, &z[jt] );
			if( cnum.i != 0.0 )
...
for( i=0; i<=20; i++ )
	{
	r = torch_cephes_response( f, gain );
	if( r <= 0.0 )
		r = -999.99;
	else
		r = 2.0 * dbfac * log( r );
 ...
 * mulitplied by amp
 */
double torch_cephes_response( f, amp )
double f, amp;
{
cmplx x, num, den, w;
double u;
...
for( j=0; j<zord; j++ )
	{
	torch_cephes_csub( &z[j], &x, &w );
	torch_cephes_cmul( &w, &den, &den );
	torch_cephes_csub( &z[j+zord], &x, &w );
	torch_cephes_cmul( &w, &num, &num );
	}
torch_cephes_cdiv( &den, &num, &w );
w.r *= amp;
w.i *= amp;
u = torch_cephes_cabs( &w );
return(u);
}
